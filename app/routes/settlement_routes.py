...
class SettlementRoutes:
    ...
    def get_group_settlements(self, group_id: int, db: Session = Depends(get_db)):
        try:
            settlement_summary = self.settlement_service.get_group_settlement_summary(db, group_id)
    ...
    def get_user_settlements(self, user_id: int, db: Session = Depends(get_db)):
        try:
            user_summary = self.settlement_service.get_user_settlements_summary(db, user_id)
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def mark_settlement_paid(self, group_id: int, request: MarkSettlementRequest, db: Session = Depends(get_db)):
        if request.amount <= 0:
            raise HTTPException(
    ...
    def get_settlement_history(self, group_id: int, db: Session = Depends(get_db)):
        try:
            history = self.settlement_service.get_settlement_history(db, group_id)
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def get_group_balances(self, group_id: int, db: Session = Depends(get_db)):
        try:
    ...
    def get_user_balances(self, user_id: int, db: Session = Depends(get_db)):
        try:
            balances = self.settlement_service.calculate_user_balances_across_groups(db, user_id)
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
