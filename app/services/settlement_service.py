...
class SettlementService:
    ...
    def calculate_user_balances_across_groups(self, db, user_id: int) -> Dict[int, float]:
        group_balances = {}
    ...
    def get_group_settlement_summary(self, db, group_id: int) -> Optional[GroupSettlement]:
    ...
    def get_user_settlements_summary(self, db, user_id: int) -> Optional[Dict]:
        ...
        total_owed_to_user = sum(balance for balance in group_balances.values() if balance > 0)
        total_user_owes = sum(abs(balance) for balance in group_balances.values() if balance < 0)
        
        group_details = {}
        if group_balances:
            groups = self.settlement_repo.get_groups_by_ids(db, list(group_balances.keys()))
    ...
    def mark_settlement_as_paid(
        ...
        try:
            ...
            description = f"Settlement payment from {from_user.username} to {to_user.username}"
    ...
    def get_settlement_history(self, db, group_id: int) -> List[Dict]:
